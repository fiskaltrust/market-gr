...
/**
 * EU member state VAT number regexes. Sourced from the official VIES FAQ
 * (https://taxation-customs.ec.europa.eu/...). The regex matches the digits
 * portion only — the two-letter country prefix is handled separately.
 *
 * Greece is the primary target. Note the VIES country code for Greece is
 * "EL", not the ISO-3166 "GR". The UI accepts either as input but always
 * normalises to "EL" before hitting the network.
 */
interface CountryDef {
  /** VIES country code (EL not GR). */
  code: string;
  /** Display name in English. */
  name: string;
  /** Regex matching the digits-only portion (no country prefix). */
  pattern: RegExp;
  /** Human-readable shape, shown as a hint. */
  hint: string;
}
...
/**
 * Normalises a user-entered VAT string. Returns `{ country, digits }` or
 * `null` if no two-letter prefix could be inferred.
 *
 * Accepts: "EL123456789", "GR123456789", "el 123 456 789", "123456789" (with
 * an explicit country dropdown), etc.
 */
function parseVatInput(
  ...
  // GR is the ISO-3166 code, EL is what VIES uses. Treat them as aliases.
...
/**
 * Shape of a successful response from the VIES REST API. Documented at
 * https://ec.europa.eu/taxation_customs/vies/#/technical-information — fields
 * marked optional may be absent depending on the member state (e.g. NL and ES
 * only return validity, no name/address).
 */
...
interface LookupResult {
  countryCode: string;
  vatNumber: string;
  /** Parsed JSON if the request returned a payload. */
  payload?: ViesResponse;
  /** Raw response text — useful for the details panel. */
  rawText?: string;
  /** HTTP status, when we got that far. */
  status?: number;
  /** Wall-clock duration in ms. */
  durationMs: number;
  /** Network/CORS error message (no HTTP status reached). */
  networkError?: string;
  /** True when the failure smells like CORS (TypeError, empty message). */
  corsLikely?: boolean;
}
...
function viesWebFormUrl(country: string, digits: string): string {
  // The VIES SPA reads the country / number from query parameters on the
  // landing page (#/vat-validation). Sending them lets the user pick up
  // exactly where the failed in-page lookup left off.
...
export default function VatLookup() {
  ...
  /** Per-session cache keyed by `${country}:${digits}`. */
  ...
  const submit = useCallback(async () => {
    ...
    try {
      ...
      // Don't cache failures — the user may retry once they switch network.
      setResult({
        countryCode: parsed.country,
        vatNumber: parsed.digits,
        durationMs: duration,
  ...
  const webFormUrl = parsed && parsed.digits
...
function splitAddress(address: string | undefined): string[] | undefined {
  ...
  // VIES separates address lines with \n. Some responses use double newlines
  // around blank fields — filter those out.
  return address
    .split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean);
...
// ----------------------------------------------------------------------
// Inline styles (kept here to match the other plugins' style)
// ----------------------------------------------------------------------
...
const statusBadgeRow: React.CSSProperties = {
  display: 'flex',
  alignItems: 'center',
  gap: 12,
  marginBottom: 14,
